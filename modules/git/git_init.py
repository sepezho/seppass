...
def git_init(message, bot):
	...
	if path.isfile(path_to_user_folder+'/user_data/ssh_key'):
		if not path.isdir(path_to_user_folder+'/main/.git'):
			...
			msg = bot.send_message(message.chat.id, 'Ваша папка уже является git репозиторием.', reply_markup = types.ReplyKeyboardRemove(selective=False))
			del_mess(msg, bot, 4)
			return
...
def clone_repo(message, bot, path_to_user_folder):
	...
	try:
		with Git().custom_environment(GIT_SSH_COMMAND=git_ssh_cmd):
			...
			msg = bot.send_message(message.chat.id, 'Все, походу.', reply_markup = types.ReplyKeyboardRemove(selective=False))
			del_mess(msg, bot, 4)
			return
