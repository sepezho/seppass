...
def git_clone(message, bot):
	...
	if os.path.isfile(path_to_ssh+'/ssh_key'):
		msg_handler = bot.send_message(message.chat.id, 'Введите ссылку на свой репозиторий.')
		bot.register_next_step_handler(msg_handler, lambda msg: clone_repo(msg, bot, path_to_ssh, path_to_user_folder))

	else:
		msg = bot.send_message(message.chat.id, 'Для начала создайте ssh ключ (при помощи getsshkey), и закинте его на свой github')
		del_mess(msg, bot, 2)
		return

def clone_repo(message, bot, path_to_ssh, path_to_user_folder):
	git_ssh_cmd = path_to_ssh + '/ssh_script.sh'
	with Git().custom_environment(GIT_SSH_COMMAND=git_ssh_cmd):
		Repo.clone_from(str(message.text), path_to_user_folder)		
